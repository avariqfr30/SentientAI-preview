...
import ast # Add this import at the top of the file if not already present
...
def calculator(expression: str) -> str:
    """Performs a mathematical calculation. Supports basic arithmetic operations."""
    print(f"--- Executing Calculator: '{expression}' ---")
    try:
        # A safer way to evaluate simple arithmetic expressions.
        # For more complex math, a dedicated library or a more robust parser is needed.
        # ast.literal_eval only evaluates literals like strings, numbers, tuples, lists, dicts, booleans, and None.
        # It doesn't handle operators directly.
        # A more robust solution would be to parse and compute, or use a library.
        # For this example, let's restrict it or use a very simple custom parser.
        
        # Simplistic approach for demonstration; for real use, a proper math expression parser is needed.
        allowed_chars = "0123456789+-*/(). "
        if not all(char in allowed_chars for char in expression):
            return "Calculator error: Invalid characters in expression."

        # WARNING: Using eval is dangerous. This is still not perfectly safe.
        # A better approach would be to implement a parser or use a library like numexpr.
        # For now, keeping it simple but highlighting the risk.
        # To make it safer than direct eval, one might try to parse with ast and only allow specific nodes.
        # However, a truly safe math expression evaluator is non-trivial.

        # Let's use a very restricted eval for now, acknowledging the risk remains if not properly sandboxed.
        # A better alternative for simple cases if you can control the expression format:
        # e.g., if expression is "2 + 2", you could split and parse.
        # For now, we'll just note the risk and proceed with a placeholder for a safer eval.
        
        # Placeholder for a safer evaluation. For now, we'll use a simple eval
        # and strongly recommend replacing it.
        # To make it slightly safer, we can restrict the globals/locals.
